...
def ExternalSourcePipeline(
    ...
    with pipe:
        ...
        if phase == "train":
            ...
            images = fn.pad(images, fill_value=0, axes=(0, 1), shape=(40, 40))
            crop_x = fn.random.uniform(range=(0.0, 8.0))
            crop_y = fn.random.uniform(range=(0.0, 8.0))
            images = fn.crop(
                images,
                crop=(32, 32),
                crop_pos_x=crop_x / 8.0,
                crop_pos_y=crop_y / 8.0,
            )
...
if __name__ == "__main__":
    loader = get_data_loader("data/CIFAR100", num_workers=2, batch_size=16, phase="train")
    counter = 0
