...
class CIFAR10Dataset(BaseDataLoader):
    ...
    def __split_train_val_data(self, trainsform):
        full_train_dataset = CIFAR10(root=self.data_dir, transform=trainsform, train=True, download=True)
    ...
    def get_test_dataset(self, test_transforms: Optional[transforms.Compose] = None) -> Dataset:
        return CIFAR10(root=self.data_dir, transform=test_transforms, train=False, download=True)


def get_data_loader(directory: str, num_workers: int, batch_size: int, val_split_ratio: float = 0.8):
    cifar10 = CIFAR10Dataset(directory=directory, val_split_ratio=val_split_ratio)
