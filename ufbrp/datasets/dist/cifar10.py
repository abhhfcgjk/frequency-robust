...
class CIFAR10Data(Dataset):
    def __init__(self, root: str, phase: str = "train", transform=None):
        ...
        self.dataset = datasets.CIFAR10(root=root, train=is_train, download=True, transform=self.transform)
...
class CIFAR10Dataset(BaseDataLoader):
    ...
    def get_train_dataset(self, train_transforms: Optional[transforms.Compose] = None) -> Dataset:
        return CIFAR10Data(root=self.data_dir, transform=train_transforms, phase="train")

    def get_val_dataset(self, val_transforms: Optional[transforms.Compose] = None) -> Dataset:
        return CIFAR10Data(root=self.data_dir, transform=val_transforms, phase="test")

    def get_test_dataset(self, test_transforms: Optional[transforms.Compose] = None) -> Dataset:
        return CIFAR10Data(root=self.data_dir, transform=test_transforms, phase="test")
...
def get_data_loader(directory: str, num_workers: int, batch_size: int, world_size: int=0, rank: Optional[int]=None):
    ...
    """
    dataset_wrapper = CIFAR10Dataset(directory, val_split_ratio=0.1)
