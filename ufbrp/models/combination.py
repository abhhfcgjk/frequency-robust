...
class RevNetDBFTT(nn.Module):
    ...
    def __reconstruct_from_coeffs(coeffs: list, keep_approx: bool = True, keep_details: bool = True) -> torch.Tensor:
        ...
        return ptwt.waverec2(modified_coeffs, WEIVLET_METHOD)
...
class ConsistencyDBFTT(nn.Module):
    def __init__(self, wavelet_level=4, wavelet_method="haar", num_classes=10, jacobian_delta=0.005, beta=30):
        super().__init__()
        self.wavelet_level = wavelet_level
        ...
        #     nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, bias=True),
        #     nn.BatchNorm2d(1024),
        #     nn.ReLU(),
        #     nn.Conv2d(in_channels=1024, out_channels=1024, kernel_size=1, bias=True),
        #     nn.BatchNorm2d(1024),
        #     nn.ReLU(),
        #     nn.Conv2d(in_channels=1024, out_channels=2048, kernel_size=1, bias=True),
        #     nn.BatchNorm2d(2048),
        #     nn.ReLU()
        #     )
    ...
    def __singular_penalty(self, module):
        penalty = 0
        for module in module.modules():
            if hasattr(module, "weight"):
                # print(module.singular_value)
                penalty += max(0, module.singular_value - self.beta)
        # print(penalty)
        return penalty
