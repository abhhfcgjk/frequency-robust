...
from attacks.autopgd import L1_projection
...
class SquareAttack:
        ...
        if self.norm == "L2":
            ...
            for counter in range(0, max(x_c, y_c)):
                delta[
                    max(counter2[0], 0) : min(counter2[0] + (2 * counter + 1), x),
                    max(0, counter2[1]) : min(counter2[1] + (2 * counter + 1), y),
                ] += 1.0 / (torch.Tensor([counter + 1]).view(1, 1).to(self.device) ** 4)
                counter2[0] -= 1
                counter2[1] -= 1
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == "Linf":
                x_best = torch.clamp(x + self.eps * self.random_choice([x.shape[0], c, 1, w]), 0.0, 1.0)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas[:, vh : vh + s, vw : vw + s] = 2.0 * self.eps * self.random_choice([c, 1, 1])
                    ...
                    loss_min[idx_to_fool] = idx_improved * loss + (1.0 - idx_improved) * loss_min_curr
                    ...
                    margin_min[idx_to_fool] = idx_improved * margin + (1.0 - idx_improved) * margin_min_curr
                    idx_improved = idx_improved.reshape([-1, *[1] * len(x.shape[:-1])])
                    x_best[idx_to_fool] = idx_improved * x_new + (1.0 - idx_improved) * x_best_curr
                ...
                for _ in range(h // s):
                    vw = sp_init + 0
                    for _ in range(w // s):
                        delta_init[:, :, vh : vh + s, vw : vw + s] += self.eta(s).view(1, 1, s, s) * self.random_choice(
                            [x.shape[0], c, 1, 1]
                        )
                        vw += s
                    vh += s

                x_best = torch.clamp(x + self.normalize(delta_init) * self.eps, 0.0, 1.0)
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x.shape[0]).to(self.device)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    norms_window_1 = (
                        (delta_curr[:, :, vh : vh + s, vw : vw + s] ** 2).sum(dim=(-2, -1), keepdim=True).sqrt()
                    )
                    ...
                    norms_windows = ((delta_curr * mask_image) ** 2).sum(dim=(-2, -1), keepdim=True).sqrt()
                    ...
                    new_deltas = (
                        new_deltas
                        / (1e-12 + (new_deltas**2).sum(dim=(-2, -1), keepdim=True).sqrt())
                        * (
                            torch.max(
                                (self.eps * torch.ones_like(new_deltas)) ** 2 - norms_image**2,
                    ...
                    x_new = torch.clamp(x_curr + self.normalize(delta_curr) * self.eps, 0.0, 1.0)
                    x_new = self.check_shape(x_new)
                ...
                for i_iter in range(self.n_queries):
                    ...
                    new_deltas_mask = torch.zeros_like(x_curr)
                    new_deltas_mask[:, :, vh : vh + s, vw : vw + s] = 1.0
                    norms_window_1 = delta_curr[:, :, vh : vh + s, vw : vw + s].abs().sum(dim=(-2, -1), keepdim=True)
                    ...
                    norms_image = self.lp_norm(x_best_curr - x_curr)
                    mask_image = torch.max(new_deltas_mask, new_deltas_mask_2)
                    norms_windows = (delta_curr * mask_image).abs().sum(dim=(-2, -1), keepdim=True)
                    ...
                    new_deltas = (
                        new_deltas
                        / (1e-12 + new_deltas.abs().sum(dim=(-2, -1), keepdim=True))
                        * (
                            torch.max(
                                self.eps * torch.ones_like(norms_image) - norms_image, torch.zeros_like(norms_image)
                            )
                            / c
                            + norms_windows
                        )
                    ...
                    r_curr = L1_projection(x_curr, delta_curr, self.eps * (1.0 - 1e-6))
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                if self.verbose:
                    print(
                        "restart {} - robust accuracy: {:.2%}".format(counter, acc.float().mean()),
