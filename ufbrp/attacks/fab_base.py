    ...
    def attack_single_run(self, x, y=None, use_rand_start=False, is_targeted=False):
        ...
        if use_rand_start:
            if self.norm == "Linf":
                ...
                x1 = (
                    im2
                    + (
                        torch.min(res2, self.eps * torch.ones(res2.shape).to(self.device)).reshape(
                            [-1, *[1] * self.ndims]
                        )
                    )
                    * t
                    / (t.reshape([t.shape[0], -1]).abs().max(dim=1, keepdim=True)[0].reshape([-1, *[1] * self.ndims]))
                    * 0.5
                )
            elif self.norm == "L2":
                ...
                x1 = (
                    ...
                    * t
                    / ((t**2).view(t.shape[0], -1).sum(dim=-1).sqrt().view(t.shape[0], *[1] * self.ndims))
                    * 0.5
                )
            elif self.norm == "L1":
                t = torch.randn(x1.shape).to(self.device)
                x1 = (
                    ...
                    * t
                    / (t.abs().view(t.shape[0], -1).sum(dim=-1).view(t.shape[0], *[1] * self.ndims))
                    / 2
                )
        ...
        while counter_iter < self.n_iter:
            with torch.no_grad():
                ...
                if self.norm == "Linf":
                    dist1 = df.abs() / (1e-12 + dg.abs().reshape(dg.shape[0], dg.shape[1], -1).sum(dim=-1))
                elif self.norm == "L2":
                    dist1 = df.abs() / (1e-12 + (dg**2).reshape(dg.shape[0], dg.shape[1], -1).sum(dim=-1).sqrt())
                elif self.norm == "L1":
                ...
                dg2 = dg[u1, ind]
                b = -df[u1, ind] + (dg2 * x1).reshape(x1.shape[0], -1).sum(dim=-1)
                w = dg2.reshape([bs, -1])

                if self.norm == "Linf":
                    d3 = projection_linf(
                        torch.cat((x1.reshape([bs, -1]), x0), 0), torch.cat((w, w), 0), torch.cat((b, b), 0)
                    )
                elif self.norm == "L2":
                    d3 = projection_l2(
                        torch.cat((x1.reshape([bs, -1]), x0), 0), torch.cat((w, w), 0), torch.cat((b, b), 0)
                ...
                a2 = a0[-bs:]
                alpha = torch.min(
                    torch.max(a1 / (a1 + a2), torch.zeros(a1.shape).to(self.device)),
                ...
                if is_adv.sum() > 0:
                    ...
                    adv[ind_adv] = x1[ind_adv] * (t < res2[ind_adv]).float().reshape([-1, *[1] * self.ndims]) + adv[
                        ind_adv
                    ] * (t >= res2[ind_adv]).float().reshape([-1, *[1] * self.ndims])
        ...
        if self.verbose:
            print(
                "success rate: {:.0f}/{:.0f}".format(ind_succ.float().sum(), corr_classified)
                + " (on correctly classified points) in {:.1f} s".format(time.time() - startt)
            )
                ...
                for target_class in range(2, self.n_target_classes + 2):
                    ...
                    for counter in range(self.n_restarts):
                        ...
                        if ind_to_fool.numel() != 0:
                            x_to_fool, y_to_fool = x[ind_to_fool].clone(), y[ind_to_fool].clone()
                            adv_curr = self.attack_single_run(
                                x_to_fool, y_to_fool, use_rand_start=(counter > 0), is_targeted=True
                            )
