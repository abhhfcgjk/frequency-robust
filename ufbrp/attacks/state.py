...
class EvaluationState:
    ...
    def to_disk(self, force: bool = False) -> None:
        ...
        if self.path is None or (seconds_since_last_save < self._SAVE_TIMEOUT and not force):
            return
    ...
    def from_disk(cls, path: Path) -> "EvaluationState":
        ...
        d["path"] = Path(d["path"])
        if path != d["path"]:
            warnings.warn(UserWarning("The given path is different from the one found in the state file."))
    ...
    def robust_accuracy(self) -> float:
        ...
        if self.attacks_to_run - self.run_attacks:
            warnings.warn("You are checking `robust_accuracy` before all the attacks" " have been run.")
