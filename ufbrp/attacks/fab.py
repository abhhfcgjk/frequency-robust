...
class FABAttackBase(Attacker):
    ...
    def __init__(
        ...
        self.n_iter = n_iter
        self.eps = eps if eps is not None else DEFAULT_EPS_DICT_BY_NORM["Linf"]
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False, is_targeted=False):
        ...
        if use_rand_start:
            ...
            x1 = (
                im2
                + (torch.min(res2, self.eps * torch.ones(res2.shape).to(self.device)).reshape([-1, *[1] * self.ndims]))
                * t
...
class FABAttack(FABAttackBase):
    ...
    def __init__(
        ...
        """FAB-attack implementation in pytorch"""
        super().__init__(model, n_restarts, n_iter, eps, alpha_max, eta, beta, targeted, device, n_target_classes)
