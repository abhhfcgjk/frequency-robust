...
def norm_loss_with_normalization(
    ...
    if batch_size > 1:
        ...
        scale = np.power(2, max(1, 1.0 / q)) * np.power(batch_size, max(0, 1.0 / p - 1.0 / q))  # p, q>0
        loss0, loss1 = 0, 0
...
class AutoPGD(Attacker):
    ...
    def run(self, inputs: Tensor, target: Tensor):
        ...
        step_size = (
            self.alpha
            * self.eps
            * torch.ones([inputs.shape[0], *[1] * (len(inputs.shape) - 1)], device=device, dtype=inputs.dtype)
        )
        ...
        for i in range(self.n_iter):
            ...
            x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1, inputs - self.eps), inputs + self.eps), 0.0, 1.0)
            x_adv_1 = torch.clamp(
                torch.min(
...
class ImageNetPGD(Attacker):
    def __init__(self, model, loss_computer, eps, alpha=None, iters=10, mode="zero",
    ...
    def run(self, inputs_norm: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            for _ in range(self.iters):
                noise_wg = noise.detach().requires_grad_(True)

                noisy01 = (x01 + noise_wg).clamp(0.0, 1.0)
                ...
                grad = torch.autograd.grad(loss, noise_wg, only_inputs=True)[0].detach()
