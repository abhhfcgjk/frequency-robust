...
def check_range_output(model, x, alpha=1e-5, logger=None):
    with torch.no_grad():
        output = model(x)
    fl = [output.max() < 1.0 + alpha, output.min() > -alpha, ((output.sum(-1) - 1.0).abs() < alpha).all()]
    if all(fl):
        msg = (
...
def check_n_classes(n_cls, attacks_to_run, apgd_targets, fab_targets, logger=None):
    msg = None
    if "apgd-dlr" in attacks_to_run or "apgd-t" in attacks_to_run:
        ...
        elif "apgd-t" in attacks_to_run and apgd_targets + 1 > n_cls:
            msg = (
